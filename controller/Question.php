...
class QuestionController extends \Xpmse\Loader\Controller {
	...
	function index() {	
		...
		App::render($data,'question','search.index');
	...
	function detail() {
		...
		if ( !empty($question_id) ) {
			$rs = $inst->getByQuestionId($question_id);
			if ( !empty($rs) ) {
	...
	function remove(){
		...
		$inst = new \Xpmsns\Qanda\Model\Question;
		$question_ids =$inst->remove( $question_id, "question_id" );
		echo json_encode(['message'=>"删除成功", 'extra'=>['$question_ids'=>$question_ids]]);
	}
